...
contract CompanyContract is SuperAppBase {
	...
	function startWorking() external isEmployed isNotWorking {
		require(
			activeInFlowRate >= (activeOutFlowRate + employees[msg.sender].salaryFlowRate),
	...
	function stopWorking() external isEmployed isWorking {
		...
		_host.callAgreement(
			_cfa,
			abi.encodeWithSelector(_cfa.deleteFlow.selector, _acceptedToken, address(this), msg.sender, new bytes(0)),
			'0x'
		);
		activeOutFlowRate -= employees[msg.sender].salaryFlowRate;
	}
	...
	function isWorkingNow() external view isEmployed returns (bool) {
		return employees[msg.sender].isWorking;
	}

	function isEmployee() external view returns (bool) {
		return employees[msg.sender].isEmployed;
	}

	function getEmployees() external view isOwner returns (Employee[] memory) {
		Employee[] memory companyEmployees = new Employee[](employeeAddresses.length);
	...
	modifier isEmployed() {
		require(_isEmployed(msg.sender), "Unable to start transaction because you're not employed");
		_;
	}

	modifier isWorking() {
		require(_isWorking(msg.sender), "Unable to start transaction because you're not employed");
		_;
	}
